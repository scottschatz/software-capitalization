...
├── package.json              # Root: npm workspaces (web, agent)
...
│       │   │   └── team/             # Team admin (placeholder)
│       │   ├── api/                  # 20 API routes
│       │   │   ├── agent/            # sync, projects, last-sync
│       │   │   ├── auth/             # NextAuth
...
│       │   ├── projects/     # PhaseBadge, ProjectForm, PhaseChangeDialog/Review
...
└── agent/                    # CLI tool (@townsquare/cap-agent)
    └── src/
        ...
        └── parsers/          # claude-jsonl, claude-scanner, git-log
```
...
### Agent: JSONL Parser
- **Purpose**: Stream-parse Claude Code JSONL files (up to 192MB) for session metrics
...
2. Agent Sync (manual or cron)
   cap sync → parse JSONL + git → POST /api/agent/sync → raw_sessions + raw_commits

3. AI Entry Generation (systemd timer, 7 AM ET)
   POST /api/entries/generate → groups raw data by developer+date → AI → daily_entries (pending)

4. Developer Review (email at 8 AM ET or web UI)
   "Approve All" email link → GET /api/email-reply/approve → confirmed
   Web UI → PATCH /api/entries/[id]/confirm → confirmed

5. Reporting
   monthly_reports aggregated from confirmed daily_entries + manual_entries
```
...
- **Immutable Raw Data**: RawSession, RawCommit, RawVscodeActivity, AgentSyncLog
